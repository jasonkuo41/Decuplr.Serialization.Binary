    ...
    static class DiagnosticHelper {
        ...
        public static DiagnosticDescriptor ExplicitTypeMushHaveConstructor { get; }
            = new DiagnosticDescriptor($"{IdTitle}-007",
                "BinaryParser explicit specifies a type that does not have corresponding constructor",
                "BinaryParser explicit specifies `{0}` to be resolved, but does not have a corresponding consturctor taking one agrument of type `{0}`",
                Category, DiagnosticSeverity.Error, true);
    }
}
